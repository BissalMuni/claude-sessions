...
- `SCREEN_TOKEN` — access token; random hex if unset (printed to console). Used only when neither `ACCOUNT_RULES` (code) nor `SCREEN_ACCESS` (env) is set.
...
- `SCREEN_DANGER` — **dangerous mode (default ON).** Set `SCREEN_DANGER=0` to start in safe mode; any other value (or unset) starts ON. When on, `canUseTool` auto-allows every tool *except* `AskUserQuestion` without prompting the phone. The only path that reaches the phone is the agent calling `AskUserQuestion` itself. A shared "constitution" is injected into every session (via `systemPrompt: { preset: 'claude_code', append }`) instructing the agent to stop and ask via `AskUserQuestion` on project-direction decisions (architecture/tech choices, ambiguous requirements, irreversible/risky actions, large scope changes). Safe mode (`SCREEN_DANGER=0`) keeps the original behavior — every tool routes through the phone for Yes/No. The env var only sets the **initial** value; danger is a **runtime toggle** (`dangerMode.ts`) flippable live from the phone (`GET`/`POST /api/danger`) and synced to all devices over WS (`danger` event + on the connect `snapshot`). `canUseTool` reads `isDanger()` per call, so a toggle takes effect on in-flight sessions immediately; the constitution's danger warning only changes for sessions started after the toggle (a live session's `systemPrompt` can't be rewritten).
...
- **Default-deny:** if the phone says No or the session aborts, the tool is denied. Safe read-only tools the SDK auto-classifies never reach the phone. **Exception — and the current default:** danger mode is **ON by default** (toggle off from the phone or with `SCREEN_DANGER=0`), inverting the gate — every tool except `AskUserQuestion` auto-allows and the agent's injected constitution is the only thing that pulls a human in. So in the default configuration the Yes/No gate does *not* hold; it only holds in safe mode. Treat the constitution (ask via `AskUserQuestion` on project-direction/irreversible decisions) as the primary human-in-the-loop safeguard.
...
- Live session state is **in-memory** (capped at `MAX_MESSAGES = 300` per session), but sessions now **survive server restarts**. `sessionStore.ts` persists a per-session index (id, title, cwd, `sdkSessionId`, message history, timestamps) to `.data/sessions.json` (debounced ~1s, flushed synchronously on shutdown). On boot `SessionManager.restore()` re-creates each saved session and passes `resume: sdkSessionId` to the SDK so the conversation continues from the SDK transcript in `~/.claude/projects/`. `sdkSessionId` is captured from the SDK `init` message; if it's null (session never got that far) the restored session starts with an empty SDK context but keeps its displayed history. Deleting a session removes it from the store too.
